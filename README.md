...
`fons.addEventListener("click", function(){ fons.style.backgroundColor = "red"; });`
...
`botoToggle.addEventListener("click", function(){ textClasse.classList.toggle("actiu"); });`
...
`botoAlerta.addEventListener("click", function(){ alert("Hola! Has fet clic al botó."); });`
...
`botoAfegir.addEventListener("click", function(){ let nouItem = document.createElement("li");` 
...
`elementEliminar.addEventListener("click", function(){ elementEliminar.remove(); });`
